...
class Encryption
{
    ...
    public static function hash($algo, $data, $returnRaw = false)
    {
        ...
        elseif (strpos(strtolower($algo), "sha3-") !== false) {
            // use Keccak instead of PHP hash()
            $hash = Sha3Hasher::hash($algo, $data->getBinary(), true);
        }
        else {
    ...
    public static function sign(KeyPair $keyPair, $data, $algorithm = "sha3-512")
    {
        ...
        $byteArray = self::toByteArray($new);
        $str = implode(array_map("chr", $byteArray));

        $privHash = self::hash($algorithm, $str, true);
